...
def get_site_improvement_report(request):
    ...
    union = request.POST.get('union')
    camp = request.POST.get('camp')
    query = """  with filtered_site_improvement as( select * from vwactivity_progress_site_improvement where upazila like '"""+str(upazila)+"""' and union_name like '"""+str(union)+"""' and camp like '"""+str(camp)+"""' and act_date::date between '-infinity'::date and '""" +str(search_date)+ """'), am as ( SELECT(SELECT distinct sub_activity_code FROM vw_all_map WHERE sub_activity_id = am.sub_activity_id and donor_id = (select donor_id from project where id = am.project_id)) , ( SELECT distinct project_code FROM vw_all_map WHERE project_id = am.project_id and sub_activity_code= (SELECT distinct sub_activity_code FROM vw_all_map WHERE sub_activity_id = am.sub_activity_id and donor_id = (select donor_id from project where id = am.project_id))), start_date::date, end_date:: date, status, upazila, union_name, camp, sum(target) s_target FROM activity_mapping am where upazila like '"""+str(upazila)+"""' and union_name like '"""+str(union)+"""' and camp like '"""+str(camp)+"""' group by sub_activity_id,project_id,start_date::date,end_date::date,status,upazila,union_name,camp )select act_level,implement_partner,(select name from camp where code = am.camp) as camp,(select distinct activity_name from vw_all_map where activity_code = activity and project_code = project_code) activity, (select distinct sub_activity_name from vw_all_map where sub_activity_code = sub_activity and project_code = project_code)sub_activity,'Meter' unit,s_target::text,trunc(sum(number::float)::numeric,2)::text num,start_date::text,end_date::text,trunc(sum(length::float)::numeric,2)::text len,status,case when s_target::bigint != 0 then trunc((sum(length::float)*100.0/s_target)::numeric,2)::text else '0' end  progress from filtered_site_improvement st,am where am.sub_activity_code::bigint = st.sub_activity::bigint and am.project_code::bigint = st.project_name::bigint and am.upazila = st.upazila and am.union_name = st.union_name and am.camp = st.camp group by act_level,implement_partner,am.camp,activity,sub_activity,s_target,start_date,end_date,status """
    print(query)
    data = json.dumps(__db_fetch_values_dict(query))
    return HttpResponse(data)

def wash_4W_report(request):
    from_date = datetime.datetime.today().date()
    return render(request, 'hcmp_report/wash_4W_report.html',{'from_date':from_date})

@csrf_exempt
def get_wash_4W_report(request):
    search_date = request.POST.get('search_date')
    query = """  with t as( select *,'Hygiene' as form_name from vwactivity_progress_wash_hygiene union all select *,'Sanitation' as form_name from vwactivity_progress_wash_sanitation union all select *,'Solid Waste' as form_name from vwactivity_progress_wash_solid_waste union all select *,'Water' as form_name from vwactivity_progress_wash_water), filtered_data AS (SELECT * FROM t WHERE act_date :: DATE BETWEEN '-infinity' :: DATE AND '"""+str(search_date)+"""'), am AS (select (SELECT DISTINCT sector_id FROM vw_all_map WHERE sub_activity_id = am.sub_activity_id AND donor_id = (SELECT donor_id FROM project WHERE id = am.project_id)),(SELECT DISTINCT activity_code FROM vw_all_map WHERE sub_activity_id = am.sub_activity_id AND donor_id = (SELECT donor_id FROM project WHERE id = am.project_id)) activity,(SELECT DISTINCT sub_activity_code FROM vw_all_map WHERE sub_activity_id = am.sub_activity_id AND donor_id = (SELECT donor_id FROM project WHERE id = am.project_id)) sub_activity, (SELECT DISTINCT project_code FROM vw_all_map WHERE project_id = am.project_id AND sub_activity_code = (SELECT DISTINCT sub_activity_code FROM vw_all_map WHERE sub_activity_id = am.sub_activity_id AND donor_id = (SELECT donor_id FROM project WHERE id = am.project_id))), start_date :: DATE, end_date :: DATE, status, upazila, union_name, camp, target FROM activity_mapping am )SELECT case when st.program_partner = '10' then 'BRAC' end program_partner, case when st.implement_partner = '10' then 'BRAC' end implement_partner, (SELECT DISTINCT donor_name FROM vw_all_map WHERE sub_activity_code = am.sub_activity AND project_code = am.project_code and donor_id = st.donor::bigint) donor ,st.form_name, (SELECT DISTINCT sub_activity_name FROM vw_all_map WHERE sub_activity_code = am.sub_activity AND project_code = am.project_code) sub_activity, (SELECT DISTINCT activity_name FROM vw_all_map WHERE activity_code = am.activity AND project_code = am.project_code) activity ,am.status, 'Chittagong' as division, (select name from district where id = st.district::bigint) district, (select name from upazila where id = st.upazila::bigint) upazila, (select name from unions where id = st.union_name::bigint) union_name, coalesce((SELECT name FROM camp WHERE code = st.camp),'') AS camp,to_char('""" +str(search_date) +"""'::date,'MM-DD-YYYY') as reporting_date,to_char(am.end_date::date,'MM-DD-YYYY') end_date, sum(st.reached_total_beneficiary::int) reached_total_beneficiary_families, sum(st.reached_child_female::int)+sum(st.reached_adult_male::int)+sum(st.reached_child_male::int) +sum(st.reached_adult_female::int)+sum(st.beneficiaries_unknown_sex_age::int) reached_total_beneficiary_individual, sum(st.reached_adult_male::int)+sum(st.reached_child_male::int) reached_total_male, sum(st.reached_adult_female::int)+sum(st.reached_child_female::int) reached_total_female, sum(st.reached_adult_female::int) reached_adult_female, sum(st.reached_adult_male::int) reached_adult_male, sum(st.reached_child_female::int) reached_child_female, sum(st.reached_child_male::int) reached_child_male, sum(st.beneficiaries_unknown_sex_age::int) beneficiaries_unknown_sex_age,'' as notes, (select contact_focal_point from sector where id = sector_id limit 1), (select phone_no from sector where id = sector_id limit 1), (select email from sector where id = sector_id limit 1), am.target,sum(total_quantity_achieved::int) total_quantity_achieved FROM filtered_data st , am where am.sub_activity :: bigint = st.sub_activity :: bigint AND am.project_code :: bigint = st.project_name :: bigint AND am.upazila = st.upazila AND am.union_name = st.union_name and case when st.camp is null and am.camp = '' then TRUE when am.camp = st.camp then TRUE else FALSE end GROUP by st.program_partner, st.implement_partner, st.donor,st.form_name,am.activity,am.sub_activity,am.project_code ,am.status,st.district,st.upazila, st.union_name,st.camp,am.end_date,sector_id,am.target """
    data = json.dumps(__db_fetch_values_dict(query))
    return HttpResponse(data)

def activity_matrix_report(request):
    from_date = datetime.datetime.today().date()
    return render(request, 'hcmp_report/activity_matrix_report.html',{'from_date':from_date})

@csrf_exempt
def get_activity_matrix_report(request):
    search_date = request.POST.get('search_date')
    target_population = request.POST.get('target_population')
    upazila = request.POST.get('upazila')
    union = request.POST.get('union')
    camp = request.POST.get('camp')
    if camp == '%':
        query = """  with t as( select *,'Hygiene' as form_name from vwactivity_progress_wash_hygiene union all select *,'Sanitation' as form_name from vwactivity_progress_wash_sanitation union all select *,'Solid Waste' as form_name from vwactivity_progress_wash_solid_waste union all select *,'Water' as form_name from vwactivity_progress_wash_water), filtered_data AS (SELECT * FROM t WHERE target_population like '""" + str(
            target_population) + """' and upazila like '""" + str(upazila) + """' and union_name like '""" + str(
            union) + """'  and act_date :: DATE BETWEEN '-infinity' :: DATE AND '""" + str(
            search_date) + """') select coalesce((SELECT name FROM camp WHERE code = camp),'') AS camp,(select name from unions where id = union_name::bigint) union_name,(SELECT DISTINCT activity_name FROM vw_all_map WHERE activity_code = activity AND project_code = project_code) activity,to_char('""" + str(
            search_date) + """'::date,'MM-DD-YYYY') as reporting_date, sum(reached_total_beneficiary::int) households, sum(reached_child_female::int)+sum(reached_adult_male::int)+sum(reached_child_male::int) +sum(reached_adult_female::int)+sum(beneficiaries_unknown_sex_age::int) persons, sum(reached_adult_female::int) adult_female, sum(reached_adult_male::int) adult_male, sum(reached_child_female::int) + sum(reached_child_male::int) children from filtered_data group by camp,union_name,activity """
    else:
        query = """  with t as( select *,'Hygiene' as form_name from vwactivity_progress_wash_hygiene union all select *,'Sanitation' as form_name from vwactivity_progress_wash_sanitation union all select *,'Solid Waste' as form_name from vwactivity_progress_wash_solid_waste union all select *,'Water' as form_name from vwactivity_progress_wash_water), filtered_data AS (SELECT * FROM t WHERE target_population like '""" +str(target_population) + """' and upazila like '""" +str(upazila) + """' and union_name like '""" +str(union) + """' and camp like '""" +str(camp) + """' and act_date :: DATE BETWEEN '-infinity' :: DATE AND '""" +str(search_date)+ """') select coalesce((SELECT name FROM camp WHERE code = camp),'') AS camp,(select name from unions where id = union_name::bigint) union_name,(SELECT DISTINCT activity_name FROM vw_all_map WHERE activity_code = activity AND project_code = project_code) activity,to_char('""" +str(search_date)+ """'::date,'MM-DD-YYYY') as reporting_date, sum(reached_total_beneficiary::int) households, sum(reached_child_female::int)+sum(reached_adult_male::int)+sum(reached_child_male::int) +sum(reached_adult_female::int)+sum(beneficiaries_unknown_sex_age::int) persons, sum(reached_adult_female::int) adult_female, sum(reached_adult_male::int) adult_male, sum(reached_child_female::int) + sum(reached_child_male::int) children from filtered_data group by camp,union_name,activity """
    data = json.dumps(__db_fetch_values_dict(query))
    return HttpResponse(data)
